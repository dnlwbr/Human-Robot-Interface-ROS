...
class GazeMapper:
    def __init__(self, ref_filename, videoCap):
        ...
        self.out_field = cv2.VideoWriter('gaze_out.mp4', self.fourcc, 30,
                                         (int(videoCap.get(cv2.CAP_PROP_FRAME_WIDTH)),
                                          int(videoCap.get(cv2.CAP_PROP_FRAME_HEIGHT))))
        self.out_ref = cv2.VideoWriter('gaze_out_ref.mp4', self.fourcc, 30, (int(self.refDimensions[1]),
                                                                                   int(self.refDimensions[0])))
        self.out_ref_nogaze = cv2.VideoWriter('no_gaze_out_ref.mp4', self.fourcc, 30,
    ...
    def map(self, img, gazepoint):
        def show(name, img, gp, radius):
            tmp = img.copy()
            cv2.circle(tmp, (gp[0], gp[1]), radius, (0, 255, 0), 5)
            tmp2 = cv2.resize(tmp, None, fx=0.4, fy=0.4)
            cv2.imshow(name, tmp2)
            self.out_field.write(tmp)

        show("field", img, gazepoint, 15)
        ...
        if np.all(ids is not None):
            ...
            if len(matching_ids) > 0:
                ...
                # cv2.imshow("no gaze", warped_nogaze)

                # Robot camera
                cv2.circle(img, (gazepoint[0], gazepoint[1]), 15, (0, 255, 0), 5)
                warped_gaze = cv2.warpPerspective(img, h, (self.ref.shape[1], self.ref.shape[0]))
                ...
                # show("stimulus", self.ref, dst[0][0], 30)
